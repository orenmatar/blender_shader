...
"""
This module provides functions to create various Image Embedder models using Siamese architectures based on VGG16 and ResNet18.
"""


def make_siamese_vgg(layers_to_take_and_size, final_emb=128, use_avg_pool=False, with_weights=True) -> nn.Module:
    ...
    # Add dense layers
    dense = nn.Sequential(
        nn.Flatten(), nn.Linear(size, 256), nn.ReLU(), nn.Linear(256, final_emb)  # Final embedding size
    )

    # Combine into a single model
    model = nn.Sequential(feature_extractor, pooling, dense)
    return model
...
def load_image_embedder_for_code(model_path):
    layer_size = (7, 256)  # that's the constant we went for
    model = make_siamese_resnet(
        layers_to_take_and_size=layer_size, final_emb=128, use_avg_pool=False, with_weights=False
    )
    projection = make_projection(input_dim=128)
    model = nn.Sequential(model, projection)
...
def load_resnet_model(model_path):
    ...
    checkpoint = torch.load(
        model_path, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"), weights_only=True
    )
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
...
def make_projection(input_dim, hidden_dim=256, output_dim=256):
    projection = nn.Sequential(
        nn.Linear(input_dim, hidden_dim),
        nn.ReLU(),
        nn.Dropout(0.1),  # Adjust dropout rate based on overfitting
...
def make_image_embedder_for_code_from_img_emb_model(model_path, freeze_base=False):
    layer_size = tuple((int(x) for x in re.findall("la_(.....)", model_path)[0].split("_")))
    model = make_siamese_resnet(
        layers_to_take_and_size=layer_size, final_emb=128, use_avg_pool=False, with_weights=False
    )
