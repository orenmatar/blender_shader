...
from Logic.NN_models.data_loaders import create_siam_images_dataloaders, evaluate_model_by_attribute_image_emb
from Logic.NN_models.image_embedders import make_siamese_vgg, make_siamese_dists, make_siamese_resnet
from Logic.NN_models.nn_models_utils import count_parameters
from Logic.training_scripts.scripts_utils import generate_checkpoint_name, log_gpu_memory, set_seed

neptune_key = os.getenv("NEPTUNE_API_TOKEN")
path = os.getenv("BLENDER_SHADER_DATA_PATH")
...
def train(
    ...
    train_loader, test_loader = create_siam_images_dataloaders(
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for batch_idx, (img1, img2, labels, attributes) in enumerate(train_loader, start=1):
            ...
            if batch_idx % auc_log_interval == 0:
                log_gpu_memory()
                eval_auc = evaluate_model_by_attribute_image_emb(model, test_loader, criterion)
