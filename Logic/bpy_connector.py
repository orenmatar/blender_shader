...
"""
The only function of interest is generate_image(btm: BlenderTreeManager, image_path: str),
made to take a BlenderTreeManager object and an image path and generate the image accordingly.
"""
...
def _set_for_texture_generation():
    """
...
def _settings_for_texture_generation(resolution=512):
    """
...
def _reset_blender():
    ...
    bpy.ops.wm.read_factory_settings(use_empty=True)  # Resets everything
    # OR force cleanup
...
def generate_image(btm: BlenderTreeManager, image_path: str, resolution:int =512):
    code = btm.generate_code(with_initialization_code=False)
    _clean_scene()
    _set_for_texture_generation()
    _settings_for_texture_generation(resolution=resolution)
    _set_file_path(image_path)
    ...
    nodes_adder = _NodesAdder(material.node_tree)
    exec(code)
    bpy.ops.render.render(write_still=True)


def check_nm_not_empty(nm: BlenderTreeManager) -> bool:
    generate_image(nm, "/tmp/tmp.png", resolution=224)
