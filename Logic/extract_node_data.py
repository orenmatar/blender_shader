...
unnecessary_props = {'location', 'width', 'height', 'warning_propagation', 'use_custom_color', 'show_texture', 'color',
                      'select',
                      'show_options', 'show_preview', 'hide', 'mute'}
vector_type = (bpy_prop_array, mathutils.Vector, mathutils.Euler)
relevant_nodes = ['CombineXYZ', 'Math', 'Mix', 'MixRGB', 'SeparateXYZ', 'Mapping', 'TexBrick',
                  'TexChecker', 'TexCoord', 'TexGabor', 'TexGradient', 'TexNoise', 'TexVoronoi', 'TexWave', 'ValToRGB',
                  'Value', 'VectorMath']
...
def get_shader_node_data():
    ...
    for node in dir(bpy.types):
        if node.startswith("ShaderNode"):  # Filter only shader nodes
            ...
            try:
                temp_node = bpy.data.node_groups.new("TempGroup", 'ShaderNodeTree').nodes.new(node)
            except:
                continue
            ...
            if node_name == 'ValToRGB':
                for i, element in enumerate(temp_node.color_ramp.elements):
                    ...
                    params[f'element_{i}'] = {'type': 'float', 'default': i, 'min': 0, 'max': 1}
            ...
            for i, input_socket in enumerate(temp_node.inputs):
                ...
                if hasattr(input_socket, "default_value"):
                    ...
                    elif isinstance(default_value, vector_type):  # Vector or Color values
                        vals.update({
                            "type": "vector",
                            "default": tuple(default_value)
                        })
                    inputs.append(vals)
            ...
            shader_nodes_data[node_name] = {'inputs': inputs, 'outputs': outputs, 'params': params}
