...
NODES_TO_SAMPLE_ADD = [
    ...
    "TexWave",
    "TexVoronoiDistance",
] # nodes that can be added randomly

"""
This module is made to create variations on a BlenderTreeManager. 
Every function that creates a variation returns a TwoWayVariationDescriptor - which is a forward step and a backward step.
The forward step is the change to the tree, and the backward step is the change to revert the tree back to the original.
E.g. if the forward step is to add the node, and the backward step is to remove it.
This means we can take a tree, slowly remove nodes from it until there is nothing, and the back-steps of this process
will be the process to generate the tree from scratch.
Sometimes more then one back-step is needed to revert the tree to the original state - e.g. when removing a node
that had several connections - we need to re-add the node and then re-add all the connections to it.
The functions only generate a variation, they do not apply it to the network.
This means we can save the instructions on a network (the TreeNetworksManager where each node represents a whole network).
The edges are the operations we need to take between the trees to get from one to the other - they are the variations' steps.
"""


def _param_request_type_to_variation_type(param_request_type: ParamRequestType) -> VariationType:
    """
...
def non_structural_changes(
    ...
    dist_vals = btm.get_all_nodes_values(change_types, return_ranges=True, not_input_values=True)
    btm.apply_distribution_limitations(dist_vals)
    ...
    old_vals = btm.get_nodes_properties(change_options)
    ...
    new_vals = btm.pick_random_values_from_dict(selection_dist)
    variation_type = _param_request_type_to_variation_type(change_types)
...
def add_random_node_on_edge(btm: BlenderTreeManager) -> TwoWayVariationDescriptor:
    """
    Add a random node on a random edge in the network
    """
    edges = list(btm.network.edges(data=True))
    ...
    node_type = btm.NODE_TYPES[new_node_type_name]
    new_node_out = node_type.get_random_output()
    new_node_in = node_type.get_random_input()
    # create a unique name for the new node, using uuid because in this context we don't know which name to give it:
    # if we take the max count of this btm and add one - it is possible that this btm is also connected to another btm
    # by an edge that adds a node with the same name. i.e. we now add "TexNoise_3", but there is another connection
    # adding "TexNoise_3" somewhere else in the network to the same btm, so there will be a duplicate. this can only be
    # fixed after the whole network exists, and we check where all the uuids are, this is done in fix_uuid_names.
    # this is a rare edge case, but still possible.
    new_node_name = f"{new_node_type_name}_{str(uuid4())}".replace("-", "_")
...
def add_random_edge(btm: BlenderTreeManager) -> Optional[TwoWayVariationDescriptor]:
    """
    ...
    nodes_to_sample = [node for node in btm.network.nodes() if node != btm.input_node_name and "Value" not in node]
    in_node = np.random.choice(nodes_to_sample)  # select a node to connect to
    in_node_type = btm.node_name_to_node_type(in_node)
    ...
    descendants = nx.descendants(btm.network, in_node)
    # do not connect to self, or to descendants (that creates a cycle)
    non_legit_nodes = descendants | {in_node, btm.output_node_name}
    possible_connections = [node for node in btm.network.nodes() if node not in non_legit_nodes]
    out_node = np.random.choice(possible_connections)
    out_node_type = btm.node_name_to_node_type(out_node)
    ...
    connected_edge = [edge for edge in btm.network.in_edges(in_node, data=True) if edge[2]["in"] == input_to_connect]
...
def remove_random_edge(btm: BlenderTreeManager) -> Optional[TwoWayVariationDescriptor]:
    """
    ...
    legit_edges = []
    for edge in edges:
        if edge[0] != btm.input_node_name:
            legit_edges.append(edge)
        else:
            target_type = btm.node_name_to_node_type(edge[1])
...
def remove_random_node(btm: BlenderTreeManager) -> Optional[TwoWayVariationDescriptor]:
    """
    Remove a random node from the network
    """
    nodes = [node for node in btm.network.nodes() if node not in [btm.input_node_name, btm.output_node_name]]
    if len(nodes) == 0:
        return None
    node_to_remove = np.random.choice(nodes)
    return create_remove_node_variation(btm, node_to_remove)


def create_remove_node_variation(btm: BlenderTreeManager, node_to_remove: str) -> TwoWayVariationDescriptor:
    """
    ...
    """
    node_type = btm.node_name_to_node_type(node_to_remove)
    all_in_edges = btm.network.in_edges(node_to_remove, data=True)
    all_out_edges = btm.network.out_edges(node_to_remove, data=True)
    ...
    node_attributes = btm.get_node_non_default_vals(node_to_remove)
    forward_step = {"remove_node_name": node_to_remove}
    backward_step = {
        "new_node_type": btm.node_name_to_node_type_name(node_to_remove),
...
def to_nothing_variation(btm: BlenderTreeManager, concat_param_change=True) -> TwoWayVariationDescriptor:
    """
    ...
    """
    btm_for_change = btm.copy()
    steps_forwards = []
    process_backwards = []
    while True:
        if len(btm_for_change.network) == 2:
            break
        node = _get_node_for_removal(btm_for_change)
        twoway_variation = create_remove_node_variation(btm_for_change, node)
        ...
        for variation in twoway_variation.steps_forward:
            apply_variation(btm_for_change, variation)
...
def apply_variation(btm: BlenderTreeManager, variation: VariationDescriptor):
    ...
    if variation.variation_type in [VariationType.SEED, VariationType.NUMERIC, VariationType.CAT_AND_NUMERIC]:
        ...
        if "edge" in step:
            ...
            btm.remove_edge(node_before, node_after, edge_in_out["in"])
            btm.add_edge(node_before, new_node_name, edge_in_out["out"], variation.step["new_node_in"])
            btm.add_edge(new_node_name, node_after, step["new_node_out"], edge_in_out["in"])

    elif variation.variation_type == VariationType.REMOVE_NODE:
        node_to_remove = step["remove_node_name"]
        btm.remove_node(node_to_remove)
        ...
        if "replacement_edge" in step:
            ...
            btm.add_edge(replacement_edge[0], replacement_edge[1], replacement_edge[2]["out"], replacement_edge[2]["in"])
        ...
        if len(connected_edge) > 0:
            ...
            btm.remove_edge(old_out_node, old_in_node, old_in_out_names["in"])
        btm.add_edge(out_node, in_node, out_to_connect, input_to_connect)

    elif variation.variation_type == VariationType.REMOVE_EDGE:
        in_node, out_node, input_to_connect, out_to_connect = step["in_node"], step["out_node"], step["in"], step["out"]
        btm.remove_edge(out_node, in_node, input_to_connect)
...
def replace_name_on_edge(edge, old_name, new_name):
    """
    Referring to the information of an edge to be removed or added (edge should really be a class!!!)
    so any edge (node1, node2, {'in': socket, 'out': socket})
    Checks if the old name is present, and changes where needed
    """
    new_edge = edge
    if old_name == edge[0]:
        new_edge = (new_name, edge[1], edge[2])
    elif old_name == edge[1]:
        new_edge = (edge[0], new_name, edge[2])
    return new_edge


def _get_node_for_removal(btm):
    """
    ...
    def sorting_key(node):
        ...
        special_score = (
            1
            if btm.input_node_name in btm.network.predecessors(node)
            and btm.output_node_name in btm.network.successors(node)
            else 0
        )
        # In-degree score
        return special_score, btm.network.in_degree(node)

    eligible_nodes = [x for x in btm.network.nodes if x not in [btm.input_node_name, btm.output_node_name]]
