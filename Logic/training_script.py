...
def log_gpu_memory():
    ...
    logging.info(f"GPU Memory Cached: {cached:.2f} MB")


def generate_checkpoint_name(save_path, epoch, **kwargs):
    # Format hyperparameters into the filename
    kwargs_names = "_".join([f"{key[:2]}_{handle_tuple(value)}" for key, value in kwargs.items()])
    filename = f"ep_{epoch}_{kwargs_names}.pt".replace(".", "_")
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (img1, img2, labels, attributes) in enumerate(train_loader, start=1):
            if batch_idx % 10 == 0:
                logging.info(f"{batch_idx}/{len(train_loader)}. Time: {round(time.time() - t, 2)}")
...
def main():
    ...
    train(
        ...
        layers_to_take=[int(x) for x in args.layers_to_take.split("_")],  # Convert the list of layers into a tuple
