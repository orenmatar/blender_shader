...
from Logic.constants import NODES_EDGES_SEP, NODES_START, EDGES_START, FROM_TO, INCREASE, DECREASE, NO_ACTION_LABEL
...
def edge_to_str(edge):
    out_socket = edge[2]["out"]
    in_socket = edge[2]["in"]
    return f"{edge[0]} ( {out_socket} -> {in_socket} ) {edge[1]}"


class BlenderTreeManager(object):
    """
    This class is responsible for managing the network, or tree, (using networkx) of nodes.
    """
    ...
    IMAGE_TOKEN = "N"
    WITH_IMAGE_TOKENS = f"A{IMAGE_TOKEN}B{IMAGE_TOKEN}"
    WITHOUT_IMAGE_TOKENS = f"A{IMAGE_TOKEN}I{IMAGE_TOKEN}"
...
nodes_adder = NodesAdder(material.node_tree)
    ...
    def copy(self) -> "BlenderTreeManager":
        """
        Copy the network manager
        """
        new_manager = BlenderTreeManager()
    ...
    @staticmethod
    def from_dict(manager_data: dict) -> "BlenderTreeManager":
        """
    ...
    @staticmethod
    def load_from_json(filename: str) -> "BlenderTreeManager":
        """
        Load the network from a json file
        """
        with open(filename, "r") as f:
            manager_data = json.load(f)
        return BlenderTreeManager.from_dict(manager_data)

    def is_empty_network(self) -> bool:
    ...
    def network_data_for_comparison(self) -> Tuple[dict, list]:
        """
    ...
    def rename_node(self, old_name: str, new_name: str):
        assert old_name in self.network.nodes
        assert new_name not in self.network.nodes
        new_network = nx.relabel_nodes(self.network, {old_name: new_name})
        self.network = new_network
        self.free_inputs[new_name] = self.free_inputs[old_name]
        del self.free_inputs[old_name]

    @staticmethod
    def find_nodes_differences(network1: "BlenderTreeManager", network2: "BlenderTreeManager") -> dict:
    ...
    @staticmethod
    def compare_networks(network1: "BlenderTreeManager", network2: "BlenderTreeManager", compare_node_properties=True, tuple_list_hack=False):
        """
        ...
        if compare_node_properties:
            ...
            # Don't know why exactly but sometimes we get node attributes with the same values but one is a tuple and
            # another is a list, so it returns False, as the networks are not equal.
            # I didn't find the source, so I'm casting everything to tuples for comparison
            if nodes1 != nodes2:
                nodes_diffs = BlenderTreeManager.find_nodes_differences(network1, network2)
                for node_name, vals in nodes_diffs.items():
                    for val_name, (val_1, val_2) in vals.items():
                        if tuple(val_2) != tuple(val_1):
                            return False
            # if we didn't find any differences, then just check the edges
            return edges1 == edges2
        node_types1 = {BlenderTreeManager.node_name_to_node_type_name(node) for node in nodes1}
        node_types2 = {BlenderTreeManager.node_name_to_node_type_name(node) for node in nodes2}
        return node_types1 == node_types2 and edges1 == edges2

    def __eq__(self, other: "BlenderTreeManager") -> bool:
    ...
    @staticmethod
    def get_updated_value(current_value: float, action: str, options_range: list):
        # move a third of the way up or down towards the end of the range
        if action == INCREASE:
            return current_value + (options_range[1] - current_value) / 3
        if action == DECREASE:
            return current_value - (current_value - options_range[0]) / 3
        if action == NO_ACTION_LABEL:
            return current_value
        raise ValueError

    def set_nodes_attributes(self, update_attributes: Union[dict, Mapping]):
        """
        ...
        for node, attributes in update_attributes.items():
            for attr_name, value in attributes.items():
                assert (
                    attr_name in self.network.nodes[node]
                ), f"Got an unexpected attribute name {attr_name} for node {node}"
                if value in [INCREASE, DECREASE, NO_ACTION_LABEL]:
                    # in case no numbers were given, but instruction to increase or decrease a bit
                    node_type = self.node_name_to_node_type(node)
                    cur_value = self.network.nodes[node][attr_name]
                    options_range = node_type.NUMERIC[attr_name].options_range
                    new_value = self.get_updated_value(cur_value, value, options_range)
                elif isinstance(value, (tuple, list)) and value[0] in [INCREASE, DECREASE, NO_ACTION_LABEL]:
                    # in case this is a vector and has instructions instead of numbers
                    node_type = self.node_name_to_node_type(node)
                    cur_values = self.network.nodes[node][attr_name]
                    options_range = node_type.NUMERIC[attr_name].options_range
                    new_value = []
                    for cur_value, v in zip(cur_values, value):
                        new_value.append(self.get_updated_value(cur_value, v, options_range))
                    new_value = tuple(new_value)
                else:
                    new_value = value
                self.network.nodes[node][attr_name] = new_value

    def get_node_non_default_vals(self, node_name: str) -> dict:
        """
    ...
    def generate_random_tree(self, n_additions=5):
        """
        Generate a random tree by adding nodes and connecting them randomly
    ...
    def get_new_node_name(self, node_type_name):
        """
        Gets a new node name by its type. Start with the count of that type + 1. For most cases this will work.
        However, we can have  'MixVector_1', 'MixVector_3', so the count will be 2, and we will try to make MixVector_3.
        So we verify that the new name does not exist, if it does, it means there is a number skipped, so remove 1 from
        the name of the new until we get to that gap value.
        """
        new_node_num = self.node_counts[node_type_name] + 1
        new_node_name = node_type_name + f"_{new_node_num}"
        while new_node_name in self.network.nodes:
            new_node_num -= 1
            new_node_name = node_type_name + f"_{new_node_num}"
    ...
    def add_node_by_type_name(self, node_type_name):
        ...
        """
        node_name = self.get_new_node_name(node_type_name)
        self.add_node_by_type_and_name(node_type_name, node_name)
        return node_name

    def update_node_counts(self):
        node_counts = Counter([BlenderTreeManager.node_name_to_node_type_name(node) for node in self.network.nodes])
        self.node_counts = defaultdict(int, node_counts)
    ...
    @staticmethod
    def node_name_to_node_type(node_name: str) -> type(Node):
        """
        Convert a node name to a node type name - simply remove the _i from the name
        """
        type_name = BlenderTreeManager.node_name_to_node_type_name(node_name)
        return BlenderTreeManager.NODE_TYPES[type_name]
    ...
    def generate_code(self, with_initialization_code=False) -> str:
        """
        Generate the python code that can be sent to Blender to generate the Blender Shader Tree for the network
        """
    ...
    def to_str(self, with_seeds=False, add_image_tokens=False, with_cur_image=True) -> str:
        """
        Convert the network to a string representation
        """
        nodes_strings = []
        for node_name in self.network.nodes:
            node = self.to_node_instance(node_name)
            nodes_strings.append(node.to_str(with_seeds=with_seeds))

        edges_strings = []
        for edge in self.network.edges(data=True):
            edges_strings.append(edge_to_str(edge))

        nm_str_code = (
            NODES_START
            + NODES_EDGES_SEP.join(nodes_strings)
            + NODES_EDGES_SEP
            + EDGES_START
            + NODES_EDGES_SEP.join(edges_strings)
        )
        if add_image_tokens:
            nm_str_code = self.add_image_tokens(nm_str_code, with_cur_image)
        return nm_str_code

    @staticmethod
    def add_image_tokens(nm_str, with_cur_image):
        """
        Add the image tokens to the network string representation: these tokens indicate whether an image embeddings
        will be added after these tokens, helping the NN model understand where the image data is located.
        """
        tokens_for_image = BlenderTreeManager.WITH_IMAGE_TOKENS if with_cur_image else BlenderTreeManager.WITHOUT_IMAGE_TOKENS
        code_with_images = nm_str.replace("NODES: ", f"NODES: {tokens_for_image} ; ")
        return code_with_images

    @staticmethod
    def from_str(network_string: str) -> "BlenderTreeManager":
        """
        Create a network manager from a string representation
        """
        nodes, edges = split_nodes_edges(network_string)
        new_nm = BlenderTreeManager()
        new_nm.initialize_network()
        for node in nodes:
            if not node:  # empty strings
                continue
            node_name, node_properties = parse_node(node)
            node_type_name = BlenderTreeManager.node_name_to_node_type_name(node_name)
            if node_type_name == BlenderTreeManager.OutputNodeNAME:  # done in initialization
                continue
            if node_type_name == BlenderTreeManager.InputNodeNAME:  # only update properties
                new_nm.set_nodes_attributes({node_name: node_properties})
                continue
            node_type = BlenderTreeManager.NODE_TYPES[node_type_name]
            node_seeds = node_type.get_node_type_params(ParamRequestType.SEED)
            # if we had seeds in the string they will be overwritten, otherwise added
            all_node_properties = node_seeds | node_properties
            node_vector_inputs = node_type.get_node_type_params(ParamRequestType.VECTOR_INPUT)
            all_node_properties = node_vector_inputs | all_node_properties
            new_nm._add_node(node_type, node_name, all_node_properties)

        for edge in edges:
            node_from, node_to, out_socket, in_socket = parse_edge(edge)
            new_nm.add_edge(node_from, node_to, out_socket, in_socket)
        new_nm.finish_network()
        return new_nm


def split_nodes_edges(data: str):
    """Splits the input string into separate node and edge sections."""
    nodes_part, edges_part = data.split(EDGES_START)
    # remove the NODES: prefix and the image tokens ANIN/ANBN
    nodes_part = (
        nodes_part.replace(NODES_START, "")
        .replace(BlenderTreeManager.WITH_IMAGE_TOKENS, "")
        .replace(BlenderTreeManager.WITHOUT_IMAGE_TOKENS, "")
    )
    return nodes_part.split(NODES_EDGES_SEP), edges_part.split(NODES_EDGES_SEP)


def parse_node(node_str: str) -> tuple[str, dict]:
    """Parses a node string to extract its name and attributes."""
    name, attr_str = node_str.split(NODE_OPEN, 1)
    name = name.strip()
    attr_str = attr_str.split(NODE_CLOSE, 1)[0].strip()

    attributes = {}
    if attr_str:
        attr_pairs = attr_str.split(PARAMETERS_SEP)
        for pair in attr_pairs:
            key, value = pair.split(NAME_VALUE_SEP, 1)
            key = key.strip()
            if VECTOR_VALUES_OPEN in value:
                value = value.replace(VECTOR_VALUES_OPEN, "").replace(VECTOR_VALUES_CLOSE, "")
                value = [float(x) for x in value.split(VECTOR_VALUES_SEP)]
            else:
                try:
...
def parse_edge(edge_str: str):
    """Parses an edge string to extract the connected nodes and sockets."""
    parts = edge_str.split(NODE_CLOSE)
    left, node_to = parts[0].strip(), parts[1].strip()
    node_from, sockets = left.split(NODE_OPEN)
    output_socket, input_socket = sockets.split(FROM_TO)
    return node_from.strip(), node_to.strip(), output_socket.strip(), input_socket.strip()
