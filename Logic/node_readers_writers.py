...
class ParamType(Enum):
    """
    Blender nodes support different types of parameters or inputs.
    """
    VECTOR = 1
    FLOAT = 2
...
def _dict_to_str_for_node_string(d):
    value_strings = []
    for name, value in d.items():
        if isinstance(value, (tuple, list)):
            list_string = VECTOR_VALUES_SEP.join([str(x) for x in value])
            value_strings.append(name + NAME_VALUE_SEP + VECTOR_VALUES_OPEN + list_string + VECTOR_VALUES_CLOSE)
        else:
            value_strings.append(name + NAME_VALUE_SEP + str(value))
    return PARAMETERS_SEP.join(value_strings)
...
class Node(object):
    ...
    def to_str(self, with_seeds=False):
        non_vector_numerics = _dict_to_str_for_node_string(
            {
                key: value
                for key, value in self.numeric.items()
                if self.NUMERIC[key].param_type != ParamType.VECTOR_INPUT
            }
        )
        categorical = _dict_to_str_for_node_string(self.categorical)
        params = [non_vector_numerics, categorical]
        if with_seeds:
            params.append(_dict_to_str_for_node_string(self.seeds))
        return self.node_name + NODE_OPEN + PARAMETERS_SEP.join([x for x in params if x]) + NODE_CLOSE
    ...
    def to_code(self, func_name: str) -> str:
        """
        Convert the node to python code that can be used to create the node in Blender.
